...
- which file and which source id (e.g. `S7` in a set generated by the skill, or a claim in the
...
| | What | Notes |
|---|---|---|
| **Portable prompt** | Keeping `portable/dont-stop-research.md` working well in ChatGPT, Gemini, and other assistants | Report which model, what broke, and the output you got |
| **Translations** | The README is bilingual EN/中文 | Skill prompts are English-only for now; translating them is a real project, discuss first |
| **Viewer** | Layout, accessibility, keyboard navigation | Keep it dependency-free apart from d3 via CDN |
| **The skill** | Improvements to the rules or formats | These change every future set — open an issue before a PR |
...
project's own invariants. Install `jsonschema` for full validation:
